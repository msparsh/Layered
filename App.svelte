<script lang="ts">
    ...
    class BujoStore {
        ...
        blockMap = new Map<
            string,
            { pageId: string; bIdx: number; block: Block }
        >();
        ...
        async init() {
            ...
            await Promise.all(
                m.regions.map(async (r: Region) => {
                    if (!this.pages[r.pageId])
                        this.pages[r.pageId] = (await FileSystemAPI.readJson(
                            `pages/${r.pageId}.json`,
                        )) || [this.createBlock()];
                }),
            );
        ...
        async executeSave() {
            ...
            try {
                ...
                await FileSystemAPI.writeJsonAtomic(
                    "meta.json",
                    $state.snapshot(this.meta),
                );
                for (const p of Array.from(this.dirtyPages)) {
                    if (this.pages[p])
                        await FileSystemAPI.writeJsonAtomic(
                            `pages/${p}.json`,
                            $state.snapshot(this.pages[p]),
                        );
                }
        ...
        createBlock = (
            ...
            depth = 0,
            type: BlockType = "text",
        ): Block => ({ id: Utils.generateId(), type, text, depth });
        rebuildIndex() {
            this.blockMap.clear();
            Object.entries(this.pages).forEach(([p, b]) =>
                b.forEach((blk, i) =>
                    this.blockMap.set(blk.id, {
                        pageId: p,
                        bIdx: i,
                        block: blk,
                    }),
        ...
        async cleanEmptySpreads() {
            let c = false;
            for (
                let i = this.meta.pageOrder.length - 1;
                i > this.currentP;
                i--
            ) {
                const pId = this.meta.pageOrder[i];
                if (
                    !this.meta.placedImages.some((img) => img.pageId === pId) &&
                    !this.meta.regions.some((r) => r.surfaceId === pId)
                ) {
        ...
        addBlock(nb: Block, id: string) {
            const c = this.getBlockCoords(id);
            if (c) {
                this.pages[c.pageId].splice(c.bIdx + 1, 0, nb);
                this.rebuildIndex();
                this.dirtyPages.add(c.pageId);
                this.requestSave();
            }
        ...
        moveBlockDOM(id: string, tId: string, ia: boolean) {
            const src = this.getBlockCoords(id),
                tgt = this.getBlockCoords(tId);
            if (!src || !tgt) return;
            const [m] = this.pages[src.pageId].splice(src.bIdx, 1),
                sA = src.pageId === tgt.pageId;
            this.pages[tgt.pageId].splice(
                tgt.bIdx - (sA && src.bIdx < tgt.bIdx ? 1 : 0) + (ia ? 1 : 0),
                0,
                m,
            );
            this.rebuildIndex();
    ...
    const pointerAction: Action<HTMLElement, PointerActionParams> = (
        node,
        { onDown, onMove, onUp, ignore },
    ) => {
        let a = false,
            d = (e: PointerEvent) => {
        ...
        return {
            destroy: () =>
                evs.forEach((ev, i) => node.removeEventListener(ev, hs[i])),
        };
    };
    ...
    const dragBehavior: Action<HTMLElement, DragBehaviorParams> = (
        node,
        { getObj, keys, min = [-Infinity, -Infinity] },
    ): any => {
    ...
    const Utils = {
        ...
        calcRectOffset: (cX: number, cY: number, r: DOMRect) => ({
            x: cX - r.left,
            y: cY - r.top,
        }),
        getBlockClasses: (t: string) =>
            `flex-1 outline-none whitespace-pre-wrap min-h-[1.5rem] bullet-text transition-all duration-300 ${CONFIG.H_STYLES[t] || "text-[1.05rem] leading-relaxed"} ${t === "completed" ? "line-through text-zinc-400 opacity-60" : ["migrated", "scheduled"].includes(t) ? "italic text-zinc-500" : ""}`.trim(),
        getHandleClasses: (isB: boolean) =>
            `absolute -left-8 top-0.5 w-7 h-7 flex items-center justify-center bullet-handle ${isB ? "opacity-100 text-zinc-500" : "opacity-0 text-zinc-300"} group-hover:opacity-100 hover:!text-black font-bold transition-all`,
        getNextType: (cT: BlockType) =>
            CONFIG.CYCLE[
                (CONFIG.CYCLE.indexOf(cT) + 1) % CONFIG.CYCLE.length
            ] || "task",
        getNextLayer: (cL: string, d: number) =>
            CONFIG.LAYERS[
                Utils.clamp(
                    CONFIG.LAYERS.indexOf(cL) + d,
                    0,
                    CONFIG.LAYERS.length - 1,
                )
            ],
        getNextOpacity: (cO: number) =>
            CONFIG.OPACITIES[
                (CONFIG.OPACITIES.indexOf(cO ?? 1) + 1) %
                    CONFIG.OPACITIES.length
            ],
    ...
    const api = (window as any).electronAPI,
        ...
        FileSystemAPI = {
            readJson: async (p: string) =>
                api ? await api.readJson(p) : (memFs.get(p) ?? null),
            writeJsonAtomic: async (p: string, d: any) =>
                api ? await api.writeJsonAtomic(p, d) : memFs.set(p, d),
            trashJson: async (p: string) =>
                api ? await api.trashJson(p) : memFs.delete(p),
        };
    ...
    const StickerBookManager = {
        ...
        async saveSticker(b64: string) {
            const id = Utils.generateId();
            await FileSystemAPI.writeJsonAtomic(`images/sticker-${id}.json`, {
                src: b64,
            });
            ...
            this.update(() => {
                ...
                if (activePackId !== "all")
                    store.meta.stickerPacks?.forEach((p) => {
        ...
        async removeSticker(id: string) {
            this.update(() => {
                store.meta.stickers = store.meta.stickers?.filter(
                    (s) => s !== id,
                );
                store.meta.stickerPacks?.forEach(
                    (p) => (p.stickers = p.stickers?.filter((s) => s !== id)),
                );
            });
        ...
        async render() {
            const l =
                activePackId === "all"
                    ? store.meta.stickers
                    : store.meta.stickerPacks?.find(
                          (p) => p.id === activePackId,
                      )?.stickers;
            loadedStickers = (
                await Promise.all((l || []).map((id) => this.loadSticker(id)))
            ).filter(Boolean) as LoadedSticker[];
        },
        async createPack(name: string) {
            if (!name?.trim()) return;
            const id = `pack-${Utils.generateId()}`;
            this.update(() => {
                store.meta.stickerPacks = [
                    ...(store.meta.stickerPacks || []),
                    { id, name: name.trim(), stickers: [] },
                ];
        ...
        async deletePack(pId: string) {
            this.update(() => {
                store.meta.stickerPacks = store.meta.stickerPacks?.filter(
                    (p) => p.id !== pId,
                );
                if (activePackId === pId)
                    store.meta.activePackId = activePackId = "all";
            });
        },
    ...
    const RegionManager = {
        ...
        start(e: PointerEvent) {
            ...
            const { x, y } = Utils.calcRectOffset(
                e.clientX,
                e.clientY,
        ...
        draw(e: PointerEvent) {
            ...
            const w = ([...Utils.qsa(".page-wrapper")] as HTMLElement[]).find(
                (w) => w.dataset.pageId === this.surfaceId,
            );
            if (!w) return;
            const { x, y } = Utils.calcRectOffset(
                e.clientX,
        ...
        async stop() {
            ...
            if (
                tempRegion.surfaceId &&
                tempRegion.w > CONFIG.MIN_REGION_SIZE &&
                tempRegion.h > CONFIG.MIN_REGION_SIZE
            ) {
                const pId = `page-${Utils.generateId()}`,
                    nb = store.createBlock();
    ...
    const EditorEngine = {
        update(id: string, u: Partial<Block>) {
            store.updateBlock(id, u);
        },
        indent(id: string, d: number) {
            const c = store.getBlockCoords(id);
            if (c)
                this.update(id, {
                    depth: Utils.clamp(c.block.depth + d, 0, CONFIG.MAX_DEPTH),
                });
        },
        updateText(id: string, cT: string) {
            ...
            if (!c) return;
            let pT = cT.replace(/\u00A0/g, " "),
                md = Object.entries(CONFIG.MD_MAP).find(([p]) =>
                    pT.startsWith(p),
                );
            if (md) {
                this.update(id, {
                    text: pT.substring(md[0].length),
                    type: md[1],
                });
                activeFocusId = id;
                activeCursorOffset = 0;
            } else this.update(id, { text: cT });
        },
        split(id: string, cP: number) {
            const c = store.getBlockCoords(id);
            if (!c) return;
            const { block: b } = c,
                nT = b.type.startsWith("h") ? "text" : b.type,
                nb = store.createBlock(b.text.slice(cP), b.depth, nT);
            this.update(id, { text: b.text.slice(0, cP) });
            store.addBlock(nb, id);
            activeFocusId = nb.id;
            activeCursorOffset = 0;
        },
        merge(id: string) {
            ...
            const p = store.pages[c.pageId][c.bIdx - 1],
                mP = p.text.length;
            this.update(p.id, { text: p.text + c.block.text });
            store.removeBlock(id);
            activeFocusId = p.id;
            activeCursorOffset = mP;
        },
        nav(id: string, d: number) {
            const c = store.getBlockCoords(id),
                t = c && store.pages[c.pageId][c.bIdx + d];
            if (t) {
                activeFocusId = t.id;
                activeCursorOffset = 0;
            }
    ...
    const autoResize: Action<HTMLTextAreaElement> = (node) => {
        ...
        node.addEventListener("input", r);
        setTimeout(r, 0);
        return {
            update: r,
            destroy: () => node.removeEventListener("input", r),
        };
    };

    const autoFocus: Action<
        HTMLTextAreaElement,
        { id: string; focusId: string | null; offset: number }
    > = (node, { id, focusId, offset }) => {
        const doFocus = (fId: string | null, off: number) => {
            if (id === fId) {
                tick().then(() => {
                    node.focus();
                    const tP = Math.min(off, node.value.length);
                    node.setSelectionRange(tP, tP);
                });
            }
        };

        doFocus(focusId, offset);

        return {
            update(params) {
                doFocus(params.focusId, params.offset);
            },
        };
    };
    const globalKey = (e: KeyboardEvent) => {
        const isI = ["TEXTAREA", "INPUT"].includes(
    ...
    const packBtnClass = (id: string) =>
        `px-3 py-1.5 rounded-lg text-xs font-semibold transition-all duration-200 active:scale-95 border ${activePackId === id ? "bg-zinc-900 text-white border-zinc-900 shadow-sm" : "bg-zinc-50 hover:bg-zinc-100 text-zinc-600 border-zinc-200/50"}`;
    ...
    onMount(() => {
        ...
        window.addEventListener(
            "beforeunload",
            () => store.saveTimeout && store.executeSave(),
        );
        return () => (d = true);
    });
</script>
...
<svelte:window
    ...
    onkeydown={(e: KeyboardEvent) => globalKey(e)}
    onpointerup={() => (draggingStickerId = null)}
    onpaste={globalPaste}
/>
...
<div
    class="fixed top-0 right-0 w-[340px] h-screen bg-white/90 backdrop-blur-lg shadow-[-10px_0_40px_rgba(0,0,0,0.04)] z-[50] transform transition-transform duration-500 ease-out border-l border-zinc-200/60 p-6 overflow-y-auto {stickerDrawerOpen
        ? 'translate-x-0'
        : 'translate-x-full'} flex flex-col"
>
    <div class="flex items-center justify-between mb-2 mt-16">
        <h2 class="text-2xl font-extrabold text-zinc-800 tracking-tight">
            Stickers 🌟
        </h2>
        <span
            class="text-xs font-semibold px-2 py-0.5 bg-zinc-100 text-zinc-600 rounded-full border border-zinc-200/60 shadow-sm"
            >{loadedStickers.length}</span
        >
    </div>
    <p class="text-xs text-zinc-500 mb-5 leading-relaxed">
        Upload images here to turn them into stickers! 👇 💡 <b
            >Drag stickers onto tabs</b
        > to organize them. Paste anywhere to add quickly! ✨
    </p>
    <div
        class="flex flex-wrap gap-1.5 items-center mb-6 pb-2 border-b border-zinc-100"
    >
        <button
            ...
            onpointerenter={(e: PointerEvent) => tDrop(e, 1)}
            onpointerleave={(e: PointerEvent) => tDrop(e, 0)}
            ...
            <button
                ...
                onpointerup={(e: PointerEvent) => {
                    ...
                    if (draggingStickerId) {
                        StickerBookManager.toggleStickerInPack(
                            draggingStickerId,
                            pack.id,
                        );
                        draggingStickerId = null;
                    ...
                    openCtx(e, [
                        {
                            ...
                            action: () =>
                                showCustomPrompt(
                                    "Rename Sticker Pack ✏️",
                                    pack.name,
                                    (n) =>
                                        StickerBookManager.renamePack(
                                            pack.id,
                                            n,
                                        ),
                                ),
                        },
                        {
                            ...
                            danger: true,
                            action: () =>
                                StickerBookManager.deletePack(pack.id),
                        },
                    ])}
        ...
        <button
            ...
            onclick={() =>
                showCustomPrompt("Create New Sticker Pack 📁", "", (n) =>
                    StickerBookManager.createPack(n),
                )}
            title="Create New Sticker Pack">＋</button
        >
    </div>
    <label
        class="block w-full cursor-pointer bg-zinc-900 text-white hover:bg-zinc-800 text-center py-3 rounded-xl font-medium text-sm mb-4 transition-all duration-300 hover:shadow-md active:scale-[0.98] border border-transparent"
        >➕ Upload Sticker<input
            type="file"
            accept="image/*"
            class="hidden"
            onchange={(e: Event) => {
                ...
                if (f) {
                    const r = new FileReader();
                    r.onload = (ev) =>
                        StickerBookManager.saveSticker(
    ...
    {#if promptVisible}
        <div
            class="relative bg-zinc-100 border border-zinc-200 rounded-xl p-4 mb-4 shadow-sm animate-[slideIn_0.2s_ease-out_forwards]"
        >
            <h3 class="text-sm font-bold text-zinc-800 mb-2">
                {promptTitle}
            </h3>
            <input
                ...
                class="w-full bg-white border border-zinc-200 text-zinc-800 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-zinc-800/10 focus:border-zinc-800 mb-3"
            ...
            <div class="flex gap-2 justify-end">
                <button
                    class="px-3 py-1.5 bg-zinc-200 hover:bg-zinc-300 text-zinc-700 rounded-lg text-xs font-bold"
                    onclick={() => (promptVisible = false)}>Cancel</button
                >
                <button
                    class="px-3 py-1.5 bg-zinc-900 hover:bg-zinc-800 text-white rounded-lg text-xs font-bold"
                    onclick={() => {
                        promptCallback?.(promptValue);
                        promptVisible = false;
                    }}>Save</button
    ...
    {#if packSelectorVisible}
        ...
        >
            <div class="flex items-center justify-between mb-3">
                <h3 class="text-sm font-bold text-zinc-800">Manage Packs 📁</h3>
                <button
                    class="text-zinc-400 hover:text-zinc-800 font-bold"
                    onclick={() => (packSelectorVisible = false)}>✕</button
                >
            </div>
            <div class="space-y-1.5 max-h-[180px] overflow-y-auto pr-1">
                {#if !store.meta.stickerPacks?.length}
                    <div class="text-xs text-zinc-500 text-center py-2">
                        No custom packs created!
                    </div>
                {:else}
                    {#each store.meta.stickerPacks as pack (pack.id)}
                        {@const isM = pack.stickers?.includes(
                            packSelectorStickerId || "",
                        )}
                        <button
                            class="w-full flex items-center justify-between p-2.5 rounded-lg border text-xs font-semibold transition-colors {isM
                                ? 'bg-zinc-900 text-white border-zinc-900'
                                : 'bg-white text-zinc-700 border-zinc-200 hover:bg-zinc-50'}"
                            onclick={() =>
                                StickerBookManager.toggleStickerInPack(
                                    packSelectorStickerId as string,
                                    pack.id,
                                )}
                        >
                            <span>{pack.name}</span>
                            <span>{isM ? "✓ Added" : "＋ Add"}</span>
                        </button>
                    {/each}
                {/if}
            </div>
    ...
    <div class="flex-1 overflow-y-auto pr-1">
        <div class="grid grid-cols-2 gap-4 pb-12">
            ...
            {#each loadedStickers as s (s.id)}
                <div
                    class="relative cursor-grab active:cursor-grabbing group flex items-center justify-center p-3 bg-white/60 hover:bg-white rounded-2xl shadow-sm border border-zinc-200/50 transition-all duration-300 {draggingStickerId ===
                    s.id
                        ? 'opacity-40 scale-95'
    ...
    {#each store.meta.pageOrder as pId, i (pId)}
        <div
            ...
            use:pointerAction={{
                ...
                onDown: (e: PointerEvent) => RegionManager.start(e),
                onMove: (e: PointerEvent) => RegionManager.draw(e),
                onUp: () => RegionManager.stop(),
            }}
            onpointerup={(e: PointerEvent) => {
                if (draggingStickerId) {
                    const s = loadedStickers.find(
                        (st) => st.id === draggingStickerId,
                    );
                    if (s) {
        ...
        >
            <div
                class="absolute bottom-8 left-0 w-full text-center text-zinc-400 text-sm font-mono tracking-widest select-none pointer-events-none"
            >
                {i + 1}
            </div>
            {#each store.meta.regions.filter((r) => r.surfaceId === pId) as r (r.id)}
                <div
                    class="region-box absolute bg-zinc-50/80 hover:bg-zinc-100/80 border transition-colors duration-300 rounded-lg flex flex-col layer-paper {focusedRegionId ===
                    r.id
                        ? 'bg-white border-zinc-800 shadow-md ring-1 ring-zinc-800/10'
                        : 'border-zinc-300 hover:border-zinc-400 shadow-sm'}"
                ...
                >
                    <div
                        use:dragBehavior={{
                            getObj: () =>
                                store.meta.regions.find((x) => x.id === r.id),
                            keys: ["x", "y"],
                        }}
                        class="h-5 bg-transparent {r.locked
                            ? 'cursor-default'
                            : 'cursor-grab active:cursor-grabbing'} rounded-t-lg flex items-center px-2 hover:bg-black/5 transition-colors group"
                    ></div>
                    <div
                        class="flex-1 py-4 pr-4 pl-10 cursor-text"
                        style="touch-action: pan-y;"
                        onclick={(e) => {
                            const target = e.target;
                            if (
                                target.closest?.(".bullet-handle") ||
                    ...
                    >
                        <div
                            ...
                            ondrop={(e: DragEvent) => {
                                ...
                                const dId =
                                    ...
                                    tB = (e.target as HTMLElement).closest(
                                        ".bullet-block",
                                    ) as HTMLElement;
                                if (dId && tB) {
                        ...
                        >
                            {#if store.pages[r.pageId]}{#each store.pages[r.pageId] as b (b.id)}
                                    <div
                                        class="relative flex items-start mb-2 bullet-block group {blockDraggingId ===
                                        b.id
                                            ? 'dragging'
                                            : ''}"
                                        data-id={b.id}
                                        style="margin-left:{b.depth *
                                            CONFIG.INDENT_PX}px"
                                        draggable={blockDraggingId === b.id}
                                        ondragstart={(e: DragEvent) => {
                                            const id = (e.target as HTMLElement)
                                                .closest(".bullet-block")
                                                ?.getAttribute("data-id");
                                            if (id) blockDraggingId = id;
                                        }}
                                    >
                                        <div
                                            class={Utils.getHandleClasses(
                                                CONFIG.CYCLE.includes(b.type),
                                            )}
                                            draggable="true"
                                            ondragstart={(e: DragEvent) => {
                                        ...
                                        <textarea
                                            use:autoFocus={{
                                                ...
                                                offset: activeCursorOffset,
                                            }}
                                            class="{Utils.getBlockClasses(
                                                b.type,
                    ...
                    {#if !r.locked}<div
                        ...
                        >
                            <div
                                class="w-2 h-2 border-r-2 border-b-2 border-zinc-400 rounded-sm pointer-events-none"
                            ></div>
                        </div>{/if}
                </div>
            {/each}
            {#if isDrawingRegion && tempRegion.surfaceId === pId}<div
            ...
            {#each store.meta.placedImages.filter((img) => img.pageId === pId) as img (img.id)}
                {#if store.loadedImages[img.id]}
                    <img
                        ...
                        oncontextmenu={(e: MouseEvent) =>
                            openCtx(e, [
                                ...
                                },
                                {
                                    ...
                                    action: () => {
                                        img.opacity = Utils.getNextOpacity(
                                            img.opacity || 1,
                                        );
                                        store.requestSave();
                                    },
                                ...
                                },
                                {
                                    ...
                                    action: () => {
                                        img.layerBucket = Utils.getNextLayer(
                                            img.layerBucket || "sticker",
                                            1,
                                        );
                                        store.requestSave();
                                    },
                        ...
                        class="draggable-image layer-{img.layerBucket ||
                            'sticker'} transition-transform duration-200 {zoomedImgId ===
                        img.id
                            ? 'scale-125'
                            : 'scale-100'}"
                        data-locked={img.locked}
                        alt="placed"
                        style="left:{img.left}px;top:{img.top}px;rotate:{img.rotation ||
                            0}deg;opacity:{img.opacity ?? 1};"
                        use:dragBehavior={{
                            getObj: () => img,
                            keys: ["left", "top"],
                        }}
...
<div
    ...
    onclick={(e: MouseEvent) => {
        const d = (e.target as HTMLElement).closest(
            ".indicator-dot",
        ) as HTMLElement;
...
>
    {#each store.meta.pageOrder as _, i}
        <span
            data-idx={i}
            class="indicator-dot cursor-pointer h-2.5 rounded-full transition-all duration-300 {i ===
            store.currentP
                ? 'w-7 bg-zinc-800'
...
{#if contextMenuVisible}
    ...
    >
        {#each contextMenuItems as item}
            <button
                class="w-full text-left px-3 py-1.5 hover:bg-zinc-100 text-sm {item.danger
...
{#if draggingStickerId && draggingStickerId !== "all"}
    ...
    {#if s}
        <img
            ...
            style="left:{pointerX - 50}px;top:{pointerY -
                50}px;width:100px;height:100px;object-fit:contain"
            alt="ghost"
        />
    {/if}
{/if}
