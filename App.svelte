<script lang="ts">
    ...
    interface Block {
        id: string; type: BlockType; text: string; depth: number;
    }
    interface Region {
        id: string; surfaceId: string; x: number; y: number; width: number; height: number; pageId: string; locked?: boolean;
        paperType?: string; layerBucket?: string;
    }
    interface PlacedImage {
        id: string; pageId: string; left: number; top: number; layerBucket: string; locked?: boolean; opacity?: number; rotation?: number; scale?: number;
    }
    interface StickerPack {
        id: string; name: string; stickers: string[];
    }
    interface MetaState {
        version?: number; currentP: number; pageOrder: string[]; stickers: string[]; placedImages: PlacedImage[]; regions: Region[]; stickerPacks: StickerPack[]; activePackId: string; scrolls?: number[];
    }
    interface LoadedSticker {
        id: string; src: string;
    }
    interface ContextMenuItem {
        label: string; danger?: boolean; action: (e?: MouseEvent) => void;
    }
    ...
    let zoomedImgId = $state<string | null>(null),
        ...
        blockDraggingId = $state<string | null>(null),
        activeFocusId = $state<string | null>(null),
        activeCursorOffset = $state(0),
        focusedRegionId = $state<string | null>(null),
        activePackId = $state("all"),
        promptVisible = $state(false),
        promptTitle = $state(""),
        promptValue = $state(""),
        promptCallback = $state<((v: string) => void) | null>(null),
        packSelectorVisible = $state(false),
        packSelectorStickerId = $state<string | null>(null),
    ...
    const CONFIG = {
        BULLETS: { task: "•", completed: "✕", migrated: ">", scheduled: "<", event: "○", note: "-" } as Record<string, string>,
        CYCLE: ["task", "completed", "migrated", "scheduled", "event", "note"] as BlockType[],
        MD_MAP: { "* ": "task", "x ": "completed", "> ": "migrated", "< ": "scheduled", "o ": "event", "- ": "note", "# ": "h1", "## ": "h2", "### ": "h3", "#### ": "h4", "##### ": "h5", "###### ": "h6" } as Record<string, BlockType>,
        H_STYLES: {
            h1: "text-4xl font-bold mt-6 mb-2", h2: "text-3xl font-bold mt-5 mb-2", h3: "text-2xl font-bold mt-4 mb-1", h4: "text-xl font-semibold mt-3 mb-1", h5: "text-lg font-semibold mt-2 mb-1", h6: "text-base font-semibold uppercase tracking-wider mt-2 mb-1 text-zinc-500",
        } as Record<string, string>,
        SAVE_TIMEOUT_MS: 500,
        LAYERS: ["background", "paper", "sticker", "floating", "focus"],
        OPACITIES: [1, 0.75, 0.5, 0.25],
        SCALES: [1, 0.5, 0.25, 1.5, 2],
    ...
    class BujoStore {
        meta = $state<MetaState>({ version: 4, currentP: 0, pageOrder: [], stickers: [], placedImages: [], regions: [], stickerPacks: [], activePackId: "all" });
        pages = $state<Record<string, Block[]>>({});
        currentP = $state(0);
        loadedImages = $state<Record<string, string>>({});
        blockMap = new Map<string, { pageId: string; bIdx: number; block: Block }>();
        ...
        async init() {
            const m: MetaState = { version: 4, currentP: 0, pageOrder: [], stickers: [], placedImages: [], regions: [], stickerPacks: [], activePackId: "all", ...((await FileSystemAPI.readJson("meta.json")) || {}) };
            ...
            await Promise.all(m.regions.map(async (r: Region) => {
                if (!this.pages[r.pageId]) this.pages[r.pageId] = (await FileSystemAPI.readJson(`pages/${r.pageId}.json`)) || [this.createBlock()];
            }));
            this.rebuildIndex();
        }
        requestSave() {
        ...
        async executeSave() {
            ...
            try {
                ...
                for (const p of Array.from(this.dirtyPages)) {
                    if (this.pages[p]) await FileSystemAPI.writeJsonAtomic(`pages/${p}.json`, $state.snapshot(this.pages[p]));
                }
        ...
        rebuildIndex() {
            this.blockMap.clear();
            Object.entries(this.pages).forEach(([p, b]) => b.forEach((blk, i) => this.blockMap.set(blk.id, { pageId: p, bIdx: i, block: blk })));
        }
        ...
        updateBlock(id: string, u: Partial<Block>) {
            const c = this.getBlockCoords(id);
            if (c) { Object.assign(c.block, u); this.dirtyPages.add(c.pageId); this.requestSave(); }
        }
        addBlock(nb: Block, id: string) {
            const c = this.getBlockCoords(id);
            if (c) { this.pages[c.pageId].splice(c.bIdx + 1, 0, nb); this.rebuildIndex(); this.dirtyPages.add(c.pageId); this.requestSave(); }
        }
        removeBlock(id: string) {
            const c = this.getBlockCoords(id);
            if (c) { this.pages[c.pageId].splice(c.bIdx, 1); this.rebuildIndex(); this.dirtyPages.add(c.pageId); this.requestSave(); }
        }
        moveBlockDOM(id: string, tId: string, ia: boolean) {
            const src = this.getBlockCoords(id), tgt = this.getBlockCoords(tId);
            if (!src || !tgt) return;
            ...
            if (!sA) this.dirtyPages.add(tgt.pageId);
            this.requestSave();
        }
        ...
        async spawnImage(src: string, cX?: number, cY?: number) {
            ...
            const img: PlacedImage = { id, pageId: this.meta.pageOrder[this.currentP], left: cX !== undefined ? cX - 50 : 100, top: cY !== undefined ? cY - 50 : 100, layerBucket: "sticker" };
    ...
    const pointerAction: Action<HTMLElement, PointerActionParams> = (node, { onDown, onMove, onUp, ignore }) => {
        let a = false,
            d = (e: PointerEvent) => {
                ...
                a = true; node.setPointerCapture(e.pointerId); onDown?.(e);
            },
            m = (e: PointerEvent) => { if (a) onMove?.(e); },
            u = (e: PointerEvent) => {
                if (!a) return;
                a = false; node.releasePointerCapture(e.pointerId); onUp?.(e);
            };
        const hs = [d, m, u, u], evs = ["pointerdown", "pointermove", "pointerup", "pointercancel"] as const;
    ...
    const dragBehavior: Action<HTMLElement, DragBehaviorParams> = (node, { getObj, keys, min = [-Infinity, -Infinity] }): any => {
        ...
        return pointerAction(node, {
            ...
            onDown: (e: PointerEvent) => {
                const o = getObj();
                if (o) {
                    sX = e.clientX; sY = e.clientY; iX = o[keys[0]]; if (keys.length > 1) iY = o[keys[1]]; e.preventDefault(); e.stopPropagation();
                }
            },
    ...
    const Utils = {
        ...
        getBlockClasses: (t: string) => `flex-1 outline-none whitespace-pre-wrap min-h-[1.5rem] bullet-text transition-all duration-300 ${CONFIG.H_STYLES[t] || "text-[1.05rem] leading-relaxed"} ${t === "completed" ? "line-through text-zinc-400 opacity-60" : ["migrated", "scheduled"].includes(t) ? "italic text-zinc-500" : ""}`.trim(),
        getHandleClasses: (isB: boolean) => `absolute -left-8 top-0.5 w-7 h-7 flex items-center justify-center bullet-handle ${isB ? "opacity-100 text-zinc-500" : "opacity-0 text-zinc-300"} group-hover:opacity-100 hover:!text-black font-bold transition-all`,
        getNextType: (cT: BlockType) => CONFIG.CYCLE[(CONFIG.CYCLE.indexOf(cT) + 1) % CONFIG.CYCLE.length] || "task",
    ...
    const StickerBookManager = {
        async update(fn: () => void) { fn(); await store.executeSave(); await this.render(); },
        async saveSticker(b64: string) {
            ...
            this.update(() => {
                ...
                if (activePackId !== "all") store.meta.stickerPacks?.forEach((p) => { if (p.id === activePackId) p.stickers = [...(p.stickers || []), id]; });
            });
        },
        ...
        async loadSticker(id: string) {
            ...
            if (f?.src) { StickerCache.set(id, f.src); return { id, src: f.src }; }
            return null;
        },
        async render() {
            const l = activePackId === "all" ? store.meta.stickers : store.meta.stickerPacks?.find((p) => p.id === activePackId)?.stickers;
        ...
        async createPack(name: string) {
            ...
            this.update(() => { store.meta.stickerPacks = [...(store.meta.stickerPacks || []), { id, name: name.trim(), stickers: [] }]; store.meta.activePackId = activePackId = id; });
        },
        ...
        async toggleStickerInPack(sId: string, pId: string) {
            this.update(() => {
                store.meta.stickerPacks?.forEach((pack) => { if (pack.stickers) pack.stickers = pack.stickers.filter((id) => id !== sId); });
                store.meta.stickerPacks?.forEach((pack) => { if (pack.id === pId && !(pack.stickers || []).includes(sId)) pack.stickers = [...(pack.stickers || []), sId]; });
            });
        },
    };
    ...
    const RegionManager = {
        ...
        start(e: PointerEvent) {
            ...
            if (!w) return;
            isDrawingRegion = true; document.body.style.userSelect = "none"; this.surfaceId = w.dataset.pageId || null;
            const { x, y } = Utils.calcRectOffset(e.clientX, e.clientY, w.getBoundingClientRect());
            this.startX = x; this.startY = y; tempRegion = { x, y, w: 0, h: 0, surfaceId: this.surfaceId };
        },
        draw(e: PointerEvent) {
            ...
            if (!w) return;
            const { x, y } = Utils.calcRectOffset(e.clientX, e.clientY, w.getBoundingClientRect());
            tempRegion = { ...tempRegion, x: Math.min(this.startX, x), y: Math.min(this.startY, y), w: Math.abs(x - this.startX), h: Math.abs(y - this.startY) };
        },
        async stop() {
            if (!isDrawingRegion) return;
            isDrawingRegion = false; document.body.style.userSelect = "";
            if (tempRegion.surfaceId && tempRegion.w > CONFIG.MIN_REGION_SIZE && tempRegion.h > CONFIG.MIN_REGION_SIZE) {
                ...
                store.meta.regions.push({ id: Utils.generateId(), surfaceId: tempRegion.surfaceId, x: tempRegion.x, y: tempRegion.y, width: tempRegion.w, height: tempRegion.h, pageId: pId, paperType: "blank", layerBucket: "paper" });
                store.pages[pId] = [nb]; store.rebuildIndex(); store.dirtyPages.add(pId); await store.executeSave(); activeFocusId = nb.id; activeCursorOffset = 0;
            }
        },
    };

    const EditorEngine = {
        update(id: string, u: Partial<Block>) { store.updateBlock(id, u); },
        indent(id: string, d: number) { const c = store.getBlockCoords(id); if (c) this.update(id, { depth: Utils.clamp(c.block.depth + d, 0, CONFIG.MAX_DEPTH) }); },
        updateText(id: string, cT: string) {
        ...
        split(id: string, cP: number) {
            const c = store.getBlockCoords(id); if (!c) return;
            const { block: b } = c, nT = b.type.startsWith("h") ? "text" : b.type, nb = store.createBlock(b.text.slice(cP), b.depth, nT);
            this.update(id, { text: b.text.slice(0, cP) }); store.addBlock(nb, id); activeFocusId = nb.id; activeCursorOffset = 0;
        },
        merge(id: string) {
            const c = store.getBlockCoords(id); if (!c || c.bIdx === 0) return;
            const p = store.pages[c.pageId][c.bIdx - 1], mP = p.text.length;
            this.update(p.id, { text: p.text + c.block.text }); store.removeBlock(id); activeFocusId = p.id; activeCursorOffset = mP;
        },
        nav(id: string, d: number) {
            const c = store.getBlockCoords(id), t = c && store.pages[c.pageId][c.bIdx + d];
            if (t) { activeFocusId = t.id; activeCursorOffset = 0; }
        },
    };
    ...
    const autoFocus: Action<HTMLTextAreaElement, { id: string; focusId: string | null; offset: number }> = (node, { id, focusId, offset }) => {
        const doFocus = (fId: string | null, off: number) => { if (id === fId) tick().then(() => { node.focus(); const tP = Math.min(off, node.value.length); node.setSelectionRange(tP, tP); }); };
        doFocus(focusId, offset); return { update(params) { doFocus(params.focusId, params.offset); } };
    };

    const globalKey = (e: KeyboardEvent) => {
        const isI = ["TEXTAREA", "INPUT"].includes(document.activeElement?.tagName || ""), l = e.key === "ArrowLeft", r = e.key === "ArrowRight";
        if ((l || r) && (!isI || e.altKey)) { e.preventDefault(); store.changePage(l ? -1 : 1, true); }
    };

    const showCustomPrompt = (t: string, d: string, cb: (v: string) => void) => {
        promptTitle = t; promptValue = d || ""; promptCallback = cb; promptVisible = true;
    ...
    const openCtx = (e: MouseEvent, items: ContextMenuItem[]) => {
        e.preventDefault(); e.stopPropagation(); contextMenuX = e.clientX; contextMenuY = e.clientY; contextMenuItems = items; contextMenuVisible = true;
    };
    ...
    const globalPaste = (e: ClipboardEvent) => {
        ...
        if (f) {
            ...
            r.onload = (ev) => { const b64 = ev.target?.result as string; StickerBookManager.saveSticker(b64); if (!stickerDrawerOpen) store.spawnImage(b64, pointerX || window.innerWidth / 2, pointerY || window.innerHeight / 2); };
            r.readAsDataURL(f);
        }
    };
    ...
    onMount(() => {
        let d = false;
        (async () => { await store.init(); if (d) return; activePackId = store.meta.activePackId || "all"; StickerBookManager.render(); store.restoreAllImages(); })();
...
<div class="fixed top-0 right-0 w-[340px] h-screen bg-white/90 backdrop-blur-lg shadow-[-10px_0_40px_rgba(0,0,0,0.04)] z-[50] transform transition-transform duration-500 ease-out border-l border-zinc-200/60 p-6 overflow-y-auto {stickerDrawerOpen ? 'translate-x-0' : 'translate-x-full'} flex flex-col">
    <div class="flex items-center justify-between mb-2 mt-16">
        ...
        <span class="text-xs font-semibold px-2 py-0.5 bg-zinc-100 text-zinc-600 rounded-full border border-zinc-200/60 shadow-sm">{loadedStickers.length}</span>
    </div>
    <p class="text-xs text-zinc-500 mb-5 leading-relaxed">Upload images here to turn them into stickers! 👇 💡 <b>Drag stickers onto tabs</b> to organize them. Paste anywhere to add quickly! ✨</p>
    ...
    {#if packSelectorVisible}
        <div class="relative bg-zinc-100 border border-zinc-200 rounded-xl p-4 mb-4 shadow-sm animate-[slideIn_0.2s_ease-out_forwards]">
            ...
            <div class="space-y-1.5 max-h-[180px] overflow-y-auto pr-1">
                {#if !store.meta.stickerPacks?.length} <div class="text-xs text-zinc-500 text-center py-2">No custom packs created!</div> {:else}
                    {#each store.meta.stickerPacks as pack (pack.id)}
                        {@const isM = pack.stickers?.includes(packSelectorStickerId || "")}
                        <button class="w-full flex items-center justify-between p-2.5 rounded-lg border text-xs font-semibold transition-colors {isM ? 'bg-zinc-900 text-white border-zinc-900' : 'bg-white text-zinc-700 border-zinc-200 hover:bg-zinc-50'}" onclick={() => StickerBookManager.toggleStickerInPack(packSelectorStickerId as string, pack.id)}><span>{pack.name}</span><span>{isM ? "✓ Added" : "＋ Add"}</span></button>
                    {/each}
                {/if}
            </div>
        </div>
    ...
    {#each store.meta.pageOrder as pId, i (pId)}
        <div class="page-wrapper w-[100vw] min-h-screen flex-shrink-0 relative flex justify-center overflow-hidden" data-page-id={pId} use:pointerAction={{ ignore: (e) => ["TEXTAREA", "BUTTON"].includes((e.target as HTMLElement).tagName) || (e.target as HTMLElement).classList.contains("bullet-text") || !!(e.target as HTMLElement).closest(".region-box") || !!(e.target as HTMLElement).closest(".draggable-image") || !!(e.target as HTMLElement).closest("#page-indicator"), onDown: (e) => RegionManager.start(e), onMove: (e) => RegionManager.draw(e), onUp: () => RegionManager.stop() }} onpointerup={(e) => { if (draggingStickerId) { const s = loadedStickers.find((st) => st.id === draggingStickerId); if (s) { const r = e.currentTarget.getBoundingClientRect(); store.spawnImage(s.src, e.clientX - r.left, e.clientY - r.top); } draggingStickerId = null; } }} ondragover={(e) => e.preventDefault()} ondrop={(e) => { e.preventDefault(); try { const d = JSON.parse(e.dataTransfer?.getData("application/json") || ""); if (d?.type === "sticker") { const r = (e.currentTarget as HTMLElement).getBoundingClientRect(); store.spawnImage(d.src, e.clientX - r.left, e.clientY - r.top); } } catch (err) {} }}>
            ...
            {#each store.meta.regions.filter((r) => r.surfaceId === pId) as r (r.id)}
                <div class="region-box absolute border transition-colors duration-300 rounded-lg flex flex-col layer-{r.layerBucket || 'paper'} paper-{r.paperType || 'blank'} {focusedRegionId === r.id ? 'border-zinc-800 shadow-md ring-1 ring-zinc-800/10' : 'border-zinc-300 hover:border-zinc-400 shadow-sm'}" data-locked={r.locked} style="left:{r.x}px;top:{r.y}px;width:{r.width}px;min-height:{r.height}px;" oncontextmenu={(e) => openCtx(e, [ { label: r.locked ? "Unlock" : "Lock", action: () => { r.locked = !r.locked; store.requestSave(); } }, { label: `Change Type (${r.paperType || 'blank'}) >`, action: (ev) => { if (ev) openCtx(ev, CONFIG.PAPER_TYPES.map(pt => ({ label: pt.charAt(0).toUpperCase() + pt.slice(1), action: () => { r.paperType = pt; store.requestSave(); } }))); } }, { label: `Layer: ${r.layerBucket || "paper"}`, action: () => { r.layerBucket = Utils.getNextLayer(r.layerBucket || "paper", 1); store.requestSave(); } }, { label: "Clear", action: () => { store.pages[r.pageId] = [store.createBlock()]; store.rebuildIndex(); store.dirtyPages.add(r.pageId); store.requestSave(); } }, { label: "Delete", danger: true, action: () => store.removeRegion(r.id) } ])}>
                    ...
                    <div class="flex-1 py-4 pr-4 pl-10 cursor-text" style="touch-action: pan-y;" onclick={(e) => { const target = e.target as HTMLElement; if (target.closest?.(".bullet-handle") || target.tagName === "TEXTAREA") return; const blocks = store.pages[r.pageId]; if (blocks && blocks.length === 1 && blocks[0].text.trim() === "") { activeFocusId = blocks[0].id; activeCursorOffset = 0; } }}>
                        <div class="min-h-full" ondragover={(e) => e.preventDefault()} ondragend={() => (blockDraggingId = null)} ondrop={(e) => { e.preventDefault(); const dId = e.dataTransfer?.getData("text/plain"), tB = (e.target as HTMLElement).closest(".bullet-block") as HTMLElement; if (dId && tB) { const r = tB.getBoundingClientRect(); store.moveBlockDOM(dId, tB.dataset.id as string, e.clientY > r.top + r.height / 2); } blockDraggingId = null; }}>
                            {#if store.pages[r.pageId]}{#each store.pages[r.pageId] as b (b.id)}
                                <div class="relative flex items-start mb-2 bullet-block group {blockDraggingId === b.id ? 'dragging' : ''}" data-id={b.id} style="margin-left:{b.depth * CONFIG.INDENT_PX}px" draggable={blockDraggingId === b.id} ondragstart={(e) => { const id = (e.target as HTMLElement).closest(".bullet-block")?.getAttribute("data-id"); if (id) blockDraggingId = id; }}>
            ...
            {#each store.meta.placedImages.filter((img) => img.pageId === pId) as img (img.id)}
                {#if store.loadedImages[img.id]}
                    <img src={store.loadedImages[img.id]} oncontextmenu={(e) => openCtx(e, [ { label: img.locked ? "Unlock" : "Lock", action: () => { img.locked = !img.locked; store.requestSave(); } }, { label: `Resize: ${img.scale ?? 1}x`, action: () => { img.scale = Utils.getNextScale(img.scale ?? 1); store.requestSave(); } }, { label: `Opacity: ${Math.round((img.opacity ?? 1) * 100)}% `, action: () => { img.opacity = Utils.getNextOpacity(img.opacity || 1); store.requestSave(); } }, { label: "Rotate", action: () => { img.rotation = ((img.rotation || 0) + 45) % 360; store.requestSave(); } }, { label: `Layer: ${img.layerBucket || "sticker"}`, action: () => { img.layerBucket = Utils.getNextLayer(img.layerBucket || "sticker", 1); store.requestSave(); } }, { label: "Delete", danger: true, action: () => store.removeImage(img.id) } ])} class="draggable-image layer-{img.layerBucket || 'sticker'} transition-transform duration-200" data-locked={img.locked} alt="placed" style="left:{img.left}px;top:{img.top}px;rotate:{img.rotation || 0}deg;opacity:{img.opacity ?? 1};scale:{(img.scale ?? 1) * (zoomedImgId === img.id ? 1.25 : 1)};" use:dragBehavior={{ getObj: () => img, keys: ["left", "top"] }} />
                {/if}
            {/each}
        </div>
    {/each}
...
<div id="page-indicator" class="fixed bottom-6 left-1/2 -translate-x-1/2 flex gap-3 z-50 px-6 py-3.5 bg-zinc-100/90 backdrop-blur-md border border-zinc-300 rounded-full shadow-[0_8px_30px_rgba(0,0,0,0.12)] items-center" onclick={(e) => { const d = (e.target as HTMLElement).closest(".indicator-dot") as HTMLElement; if (d) store.changePage(parseInt(d.dataset.idx || "0"), false); }}>
    {#each store.meta.pageOrder as _, i}
        <span data-idx={i} class="indicator-dot cursor-pointer h-2.5 rounded-full transition-all duration-300 {i === store.currentP ? 'w-7 bg-zinc-800' : 'w-2.5 bg-zinc-300 hover:bg-zinc-400'}"></span>
    {/each}
</div>
...
{#if contextMenuVisible}
    <div class="fixed bg-white border border-zinc-200 shadow-xl rounded-md py-1 z-[100] w-48" style="left:{contextMenuX}px;top:{contextMenuY}px">
        {#each contextMenuItems as item}
            <button class="w-full text-left px-3 py-1.5 hover:bg-zinc-100 text-sm {item.danger ? 'text-red-600 font-medium' : 'text-zinc-800'}" onclick={(e) => { e.stopPropagation(); contextMenuVisible = false; item.action(e); }}>{item.label}</button>
        {/each}
    </div>
{/if}
...
{#if draggingStickerId && draggingStickerId !== "all"}
    ...
    {#if s} <img src={s.src} class="fixed pointer-events-none z-[9999] opacity-75 scale-110 drop-shadow-lg" style="left:{pointerX - 50}px;top:{pointerY - 50}px;width:100px;height:100px;object-fit:contain" alt="ghost" /> {/if}
{/if}
...
<style>
    ...
    /* Paper Types */
    :global(.paper-blank) { background-color: rgba(250, 250, 250, 0.9); }
    :global(.paper-dot) { background: radial-gradient(#d4d4d8 1.5px, transparent 1.5px) 0 0 / 24px 24px rgba(250, 250, 250, 0.9); }
    :global(.paper-ruled) { background: repeating-linear-gradient(transparent, transparent 27px, #e4e4e7 27px, #e4e4e7 28px) 0 0 / 100% 28px rgba(250, 250, 250, 0.9); }
    :global(.paper-graph) { background: repeating-linear-gradient(#e4e4e7 0 1px, transparent 1px 100%) 0 0 / 24px 24px, repeating-linear-gradient(90deg, #e4e4e7 0 1px, transparent 1px 100%) 0 0 / 24px 24px rgba(250, 250, 250, 0.9); }
</style>