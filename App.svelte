<script lang="ts">
  ...
  interface Region { id: string; surfaceId: string; x: number; y: number; width: number; height: number; pageId: string; locked?: boolean; }
  interface PlacedImage { id: string; pageId: string; left: number; top: number; layerBucket: string; locked?: boolean; opacity?: number; rotation?: number; }
  ...
  const CONFIG = {
    ...
    H_STYLES:{h1:"text-4xl font-bold mt-6 mb-2",h2:"text-3xl font-bold mt-5 mb-2",h3:"text-2xl font-bold mt-4 mb-1",h4:"text-xl font-semibold mt-3 mb-1",h5:"text-lg font-semibold mt-2 mb-1",h6:"text-base font-semibold uppercase tracking-wider mt-2 mb-1 text-zinc-500"} as Record<string,string>,
    SAVE_TIMEOUT_MS:500,LAYERS:["background","paper","sticker","floating","focus"],OPACITIES:[1,0.75,0.5,0.25],INDENT_PX:28,MIN_REGION_SIZE:50,MAX_DEPTH:4
  };

  class BujoStore{
    meta=$state<MetaState>({version:4,currentP:0,pageOrder:[],stickers:[],placedImages:[],regions:[],stickerPacks:[],activePackId:"all"});
    pages=$state<Record<string,Block[]>>({});currentP=$state(0);loadedImages=$state<Record<string,string>>({});
    blockMap=new Map<string,{pageId:string,bIdx:number,block:Block}>();dirtyPages=new Set<string>();isSaving=false;needsSave=false;saveTimeout:any=null;
    async init(){
      ...
      await Promise.all(m.regions.map(async (r:Region)=>{if(!this.pages[r.pageId])this.pages[r.pageId]=await FileSystemAPI.readJson(`pages/${r.pageId}.json`)||[this.createBlock()];}));
      this.rebuildIndex();
    }
    ...
    async executeSave(){
      ...
      try{
        ...
        for(const p of Array.from(this.dirtyPages)){if(this.pages[p])await FileSystemAPI.writeJsonAtomic(`pages/${p}.json`,$state.snapshot(this.pages[p]));}
  ...
  const pointerAction: Action<HTMLElement, PointerActionParams> = (node,{onDown,onMove,onUp,ignore})=>{
    let a=false, d=(e:PointerEvent)=>{if(e.button!==0||ignore?.(e))return;a=true;node.setPointerCapture(e.pointerId);onDown?.(e);}, m=(e:PointerEvent)=>{if(a)onMove?.(e);}, u=(e:PointerEvent)=>{if(!a)return;a=false;node.releasePointerCapture(e.pointerId);onUp?.(e);};
  ...
  const Utils={
    generateId:()=>crypto.randomUUID(), clamp:(v:number,m:number,x:number)=>Math.max(m,Math.min(x,v)), calcRectOffset:(cX:number,cY:number,r:DOMRect)=>({x:cX-r.left,y:cY-r.top}),
    ...
    getNextLayer:(cL:string,d:number)=>CONFIG.LAYERS[Utils.clamp(CONFIG.LAYERS.indexOf(cL)+d,0,CONFIG.LAYERS.length-1)],
  ...
  const api=(window as any).electronAPI, memFs=new Map<string,any>(), FileSystemAPI={
    readJson:async(p:string)=>api?await api.readJson(p):memFs.get(p)??null, writeJsonAtomic:async(p:string,d:any)=>api?await api.writeJsonAtomic(p,d):memFs.set(p,d), trashJson:async(p:string)=>api?await api.trashJson(p):memFs.delete(p)
  };

  const StickerBookManager = {
    async update(fn:()=>void){ fn(); await store.executeSave(); this.render(); },
    async saveSticker(b64:string){ const id=Utils.generateId(); await FileSystemAPI.writeJsonAtomic(`images/sticker-${id}.json`,{src:b64}); StickerCache.set(id,b64); this.update(()=>{ store.meta.stickers=[...(store.meta.stickers||[]),id]; if(activePackId!=="all") store.meta.stickerPacks?.forEach(p=>{if(p.id===activePackId)p.stickers=[...(p.stickers||[]),id];}); }); },
    async removeSticker(id:string){ this.update(()=>{ store.meta.stickers=store.meta.stickers?.filter(s=>s!==id); store.meta.stickerPacks?.forEach(p=>p.stickers=p.stickers?.filter(s=>s!==id)); }); StickerCache.delete(id); await FileSystemAPI.trashJson(`images/sticker-${id}.json`); },
    async loadSticker(id:string){ if(StickerCache.has(id))return{id,src:StickerCache.get(id)}; const f=await FileSystemAPI.readJson(`images/sticker-${id}.json`); if(f?.src){StickerCache.set(id,f.src);return{id,src:f.src};} return null; },
    async render(){ const l = activePackId==="all" ? store.meta.stickers : store.meta.stickerPacks?.find(p=>p.id===activePackId)?.stickers; loadedStickers=(await Promise.all((l||[]).map(id=>this.loadSticker(id)))).filter(Boolean) as LoadedSticker[]; },
  ...
  const EditorEngine={
    ...
    split(id:string,cP:number){const c=store.getBlockCoords(id);if(!c)return;const {block:b}=c,nT=b.type.startsWith("h")?"text":b.type,nb=store.createBlock(b.text.slice(cP),b.depth,nT);this.update(id,{text:b.text.slice(0,cP)});store.addBlock(nb,id);activeFocusId=nb.id;activeCursorOffset=0;},
    merge(id:string){const c=store.getBlockCoords(id);if(!c||c.bIdx===0)return;const p=store.pages[c.pageId][c.bIdx-1],mP=p.text.length;this.update(p.id,{text:p.text+c.block.text});store.removeBlock(id);activeFocusId=p.id;activeCursorOffset=mP;},
  ...
  const tDrop=(e:PointerEvent,on:boolean)=>{if(draggingStickerId&&draggingStickerId!=="all")(e.currentTarget as HTMLElement).classList[on?'add':'remove']("!bg-zinc-800","!text-white","scale-105","!border-zinc-800");};
...
<div class="fixed top-0 right-0 w-[340px] h-screen bg-white/90 backdrop-blur-lg shadow-[-10px_0_40px_rgba(0,0,0,0.04)] z-[50] transform transition-transform duration-500 ease-out border-l border-zinc-200/60 p-6 overflow-y-auto {stickerDrawerOpen?'translate-x-0':'translate-x-full'} flex flex-col">
  ...
  <div class="flex flex-wrap gap-1.5 items-center mb-6 pb-2 border-b border-zinc-100">
    ...
    {#each store.meta.stickerPacks||[] as pack (pack.id)}
      <button class={packBtnClass(pack.id)} onclick={()=>{activePackId=pack.id;store.meta.activePackId=pack.id;store.requestSave();}} onpointerenter={(e:PointerEvent)=>tDrop(e,1)} onpointerleave={(e:PointerEvent)=>tDrop(e,0)} onpointerup={(e:PointerEvent)=>{tDrop(e,0);if(draggingStickerId){StickerBookManager.toggleStickerInPack(draggingStickerId,pack.id);draggingStickerId=null;}}} oncontextmenu={(e:MouseEvent)=>openCtx(e,[{label:"Rename Pack ✏️",action:()=>showCustomPrompt("Rename Sticker Pack ✏️",pack.name,n=>StickerBookManager.renamePack(pack.id,n))},{label:"Delete Pack 🗑️",danger:true,action:()=>StickerBookManager.deletePack(pack.id)}])} title="Right-click to rename/delete. Drag stickers here to add.">{pack.name}</button>
    {/each}
  ...
  {#if promptVisible}
    <div class="relative bg-zinc-100 border border-zinc-200 rounded-xl p-4 mb-4 shadow-sm animate-[slideIn_0.2s_ease-out_forwards]">
      ...
      <input id="custom-prompt-input" type="text" bind:value={promptValue} class="w-full bg-white border border-zinc-200 text-zinc-800 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-zinc-800/10 focus:border-zinc-800 mb-3" placeholder="Enter text..." onkeydown={(e:KeyboardEvent)=>{if(e.key==="Enter"){e.preventDefault();promptCallback?.(promptValue);promptVisible=false;}else if(e.key==="Escape"){e.preventDefault();promptVisible=false;}}}/>
    ...
    <div class="relative bg-zinc-100 border border-zinc-200 rounded-xl p-4 mb-4 shadow-sm animate-[slideIn_0.2s_ease-out_forwards]">
      ...
      <div class="space-y-1.5 max-h-[180px] overflow-y-auto pr-1">
        ...
        {:else} {#each store.meta.stickerPacks as pack (pack.id)}
          ...
          <button class="w-full flex items-center justify-between p-2.5 rounded-lg border text-xs font-semibold transition-colors {isM?'bg-zinc-900 text-white border-zinc-900':'bg-white text-zinc-700 border-zinc-200 hover:bg-zinc-50'}" onclick={()=>StickerBookManager.toggleStickerInPack(packSelectorStickerId as string,pack.id)}>
